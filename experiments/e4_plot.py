...
from figstyle import tidy, panel, legend_below, save
...
lo, hi = 500, 4000
h_one, = a.plot([lo, hi], [lo, hi], "--", lw=1.0, color="0.3")
a.plot([lo, hi], [lo / 1.39, hi / 1.39], ":", lw=0.9, color=C_CAP)
a.text(2350, 2350 / 1.39 - 420, "Mean 1.39", fontsize=8, color=C_CAP)
a.plot([lo, hi], [lo / 0.45, hi / 0.45], ":", lw=0.9, color=C_BEAM)
a.text(1250, 1250 / 0.45 + 150, "Mean 0.45", fontsize=8, color=C_BEAM)
h_cap = a.scatter([c[1] for c in caps], [c[2] for c in caps], s=34, marker="o", color=C_CAP,
                  edgecolor="white", lw=0.5, zorder=5)
h_beam = a.scatter([bm[1] for bm in beams], [bm[2] for bm in beams], s=34, marker="s", color=C_BEAM,
                   edgecolor="white", lw=0.5, zorder=5)
for sid, pe, pc in caps:
    a.annotate(sid, (pe, pc), fontsize=8, xytext=(4, -7), textcoords="offset points", color="0.25")
...
b.text(-0.3, 1.74, f"Mean {np.mean(ratios_cap):.2f}, CoV {np.std(ratios_cap)/np.mean(ratios_cap):.2f}",
       ha="left", va="center", fontsize=8, color=C_CAP)
...
legend_below(fig, [h_cap, h_beam, h_one], ["Pier caps (5)", "Deep beams (8)", "1:1 line"], ncol=3, y=0.005)
